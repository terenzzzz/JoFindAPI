const {full_lyric} = require('../lyric/sampleData')
const {customStopWords} = require('../lyric/stopWords')
...
function createTrainingData(corpus) { // 创建训练数据
    console.info("Start Create Training Data ...");
    const windowSize = 3; //每个目标单词选择上下文单词时的窗口大小
    ...
    console.info("Created Training Data");
...
async function trainWord2Vec(corpus) { // 创建训练模型
    console.info("Start Train Word2Vec ...");
    ...
    console.info(`Done! Saved wordIndex Path: wordIndex_${toDate()}.json`);
    ...
    console.info(`Done! Saved Model Path: ${filePath}`);
...
async function train(){
    try {
        ...
        tracks.forEach(track => {
            if(track.lyric){
                corpus.push(normalisation(track.lyric, true));
            }
